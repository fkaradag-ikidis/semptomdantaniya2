 ...
 * VERSİYON: 2.4 — 2026-04-14
 * ============================================================
 */
console.log('✅ Algoritma v2.4 yüklendi');
...
function getLabReference(id) {
    const refs = {
        ...
        'labTrig': { min: 0, max: 150, name: 'Trigliserid' },
        'labT4': { min: 0.8, max: 1.8, name: 'T4' }
    };
            ...
            if (anyLabEntered) {
                ...
                if (userProfile.length > 0) {
                    ...
                    userProfile.forEach(abn => {
                        ...
                        // Hem orijinal metin hem de [sayısal değer] köşeli parantez içini ara
                const matched = abn.keywords.some(kw => {
                    const kwl = kw.toLowerCase();
                    // Doğrudan metin eşleşmesi
                    if (disLabText.includes(kwl)) return true;
                    // Sayısal karşılaştırma: labFindings içindeki "[X >5 mg/L]" gibi ifadeler
                    return false;
                });
                        if (matched) {
                            hits++;
